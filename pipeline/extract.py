...
FESTIVAL_WORD = re.compile(
    ...
    # それ自体が行事を指す物の語。桜・梅と違って地名や名所にならないので
    # 「祭」が無くても通す。栃木の全クロールで確認: 鯉のぼり (吹上公民館の
    # 鯉のぼり)・流し雛 (思川の流し雛)・キャンドルナイト・かかしコンテスト は
    # 全件が行事だった。ライトアップ・イルミネーション・達磨・初詣・ホタル・
    # 風鈴は点灯時間・美術品・名所が混ざるので入れない (2026-09-12)。
    r"鯉のぼり|こいのぼり|鯉幟|流し雛|ひなまつり|雛まつり|ひな祭|七夕|たなばた|"
    r"どんど|とんど|大凧|凧揚げ|凧あげ|キャンドルナイト|かかしコンテスト|竹あかり"
)
...
NOT_A_NAME = re.compile(
    r"[。、！？]|"
    # 添付ファイルや配布物の名前 (「桜まつりチラシ.pdf」「協賛申込書」)。
    # 公開データで12件、すべて配布物だった (2026-09-12)。
    r"\.(?:pdf|PDF|xlsx?|docx?|jpe?g|png)\b|チラシ|ちらし|ポスター|申込書|"
    ...
    r"春祭り|花火大会|花火|イベント|フェスティバル|フェスタ|前夜祭|正月行事|"
    r"年中行事|伝統行事|郷土芸能|民俗芸能|文化祭|市民祭|区民祭|"
    # 2026-09-12 の監査で足した一般語。種目名 (神楽・獅子舞・盆踊り・太鼓) は上の理由で入れない
    r"夏まつり|秋まつり|春まつり|納涼祭|収穫祭|感謝祭|産業祭|祭り・祭礼|祭礼行事)$|"
    # 説明文 (「八甫地区に伝わる7月24日に近い日曜日に行われる獅子舞」
    # 「御輿の渡御や山車の引き廻しが行われる」)。名称ではなく紹介文の切れ端。
    # 公開データ 8件・埼玉 6件、すべて説明文だった (2026-09-11)。
    r"(?:に伝わる|に行われる|が行われる|で行われる|に開催される|が開催される)"
)
...
# 観光協会の年間行事一覧はカード全体が1つのアンカーで、文字列が
# 「3月中旬～5月中旬 うずまの鯉のぼり」の形になる。この「～」を名称と説明の
# 区切りとして切ると「3月中旬」だけが残る (栃木市で実際に落とした)。
# 先頭の期日表現は、区切りを探す前に落とす。
_LEADING_DATE = re.compile(
    r"^(?:(?:[01]?\d月(?:\d{1,2}日)?(?:[上中下]旬)?|[上中下]旬)(?:に近い日曜日)?"
