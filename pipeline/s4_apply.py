...
_NAME_FIXES: list[tuple[re.Pattern[str], str]] = [
    ...
    # FAQ の見出し「Q. 阿波おどり」「Q：通町のえびす祭り」(徳島市の観光 FAQ)
    (re.compile(r"^[QＱ][.．:：]\s*"), ""),
    ...
    # 先頭のISO日付 「~ 2026-10-25 イベント 東京味わいフェスタ」(千代田区観光協会)。
    # 先頭の「~」を飾り記号として落とした後の「2026-10-25 …」と、年だけ落ちて
    # 残る「-10-25 …」の両方に当てる。年の規則より先に置くこと (1)
    ...
    # ---- 2026-09-25 の東京の差分で足した規則。件数は公開データ 20,195 件での実測 ----
    # 末尾の巻数 「シネマフェスタかぶらVol.9」「盆踊り vol.01」(6)。回数は毎年変わる
    (re.compile(r"\s*[Vv][Oo][Ll]\.?\s*[0-9]+\s*$"), ""),
    # 一覧表の行末に残る連番 「各町神輿連合渡御 7」「仲秋祭 11」(11)。
    # 先頭の連番 (154行目) と同じ一覧由来だが、名称の後ろに付く形
    (re.compile(r"(?<=\S)\s+[0-9]{1,2}$"), ""),
    # 告知語のうち「〜の変更」形 「若郷盆祭の開催内容の変更」(1)
    (re.compile(r"\s*の?(?:開催)?(?:内容|日程|時間|会場)の(?:一部)?変更\s*$"), ""),
]
