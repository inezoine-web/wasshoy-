...
# 年は「2026」「R8」「令和8（2026）年度」の形でも落とす (2026-09-12: 公開データで年違いの
# 同名が 216 組あった)。4桁は西暦らしい 20xx だけにして、「百年後芸術祭」等を巻き込まない。
_YEAR = re.compile(r"(?:令和|平成|R|H)\s*[0-9０-９]+(?:[（(]20\d\d[）)])?\s*年?度?|20\d\d\s*年?度?")
# 先頭の日付見出し 「8月3日（土）」「4/17」「22日(日)」も照合キーからは落とす
_LEAD_DATE = re.compile(
    r"^(?:[0-9０-９]{1,2}月(?:[0-9０-９]{1,2}日)?(?:[（(][^）)]*[）)])?"
    r"|[0-9０-９]{1,2}[/／.][0-9０-９]{1,2}|[0-9０-９]{1,2}日(?:[（(][^）)]*[）)])?)\s*")
...
# ふりがなは**ひらがな**で書かれる。カタカナの括弧書きは読みではなく
# 外来語のラベルであることが多い。実際に「からかさ万灯（フォトギャラリー）」
# と「土浦桜まつり（フォトギャラリー）」の両方が `fotogyarari` という
# 同一のIDになる事故が起きた。カタカナは読みとして採らない。
_PAREN_KANA = re.compile(r"[（(]\s*([ぁ-ゖー\s]{2,30})\s*[)）]")
