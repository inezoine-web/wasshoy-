...
def strong_key(name: str) -> str:
    ...
    連体助詞の「の」も落とす。東文研の指定名称は「羽根尾の獅子舞」「鎌数の神楽」の
    形を取り、同じ行事を自治体・観光協会は「羽根尾獅子舞」「鎌数神楽」と書くため、
    同一市町村に2件できる (2026-09-25 の全件監査で60組120件)。

    **`dedup_key` 側には入れない。** あちらは `s4_apply.stable_id` のハッシュIDの
    種でもあるので、変えると同じ行事を次に拾ったとき別IDになり、重複が増える。
    こちらは S5c で `--dry-run` を見てから当てる工程なので、取りこぼしても気づける。
    """
    k = _DECOR.sub("", name)
    return dedup_key(k).replace("の", "")
