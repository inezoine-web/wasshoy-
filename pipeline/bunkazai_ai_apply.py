...
# 先頭の助詞で弾く規則も**使わない**。日本語に語境界が無いので、
# 「から」かさ万灯 (下妻市・県指定無形民俗文化財) や 「と」んだやばやし が
# 巻き込まれる。実際に「からかさ万灯」を落とした。AIは既に「これは名前だ」と
# 判断しており、カードは断片を禁じている。ここで見るのは体裁だけにする。
AI_PROSE = re.compile(
    r"https?|www\.|本文へ|お問い合わせ|問合せ|ダウンロード|ページの先頭|"
    # 文になっているもの
    r"[。、！？]|(?:します|ました|しています|ください|されており|について)$"
)
